...
class CameraTrafficLightDetector:
    ...
    def local_path_callback(self, local_path_msg):
        ...
        if len(local_path_msg.waypoints)>0:
            local_path_linestring = LineString([(waypoint.position.x, waypoint.position.y)for waypoint in local_path_msg.waypoints])
    ...
    def camera_image_callback(self, camera_image_msg):
        ...
        with self.lock:
            ...
            transform_from_frame = self.transform_from_frame #map_frame
            transform_to_frame = camera_image_msg.header.frame_id #camera_frame
            ...
            if stoplines_on_path is not None:
                ...
                if len(rois)>0:
                    ...
                    rospy.loginfo(f'''
                                stopline_on_path: {stoplines_on_path}
                                rois: {rois}
        ...
        for linkId in stoplines_on_path:
            for plId, traffic_lights in self.trafficlights[linkId].items():
                ...
                for x, y, z in traffic_lights.values():
                    ...
                    # transform point in camera frame to pixel coordinates
                    ...
                    if u < 0 or u >= self.camera_model.width or v < 0 or v >= self.camera_model.height:
                        rospy.logerr_throttle(10, "%s - Out of image, skipped", rospy.get_name())
                        break

                    # convert the extent in meters to extent in pixels
                    extent_x_px = self.camera_model.fx() * self.roi_width_extent / point_camera.z #focal length fx
                    extent_y_px = self.camera_model.fy() * self.roi_height_extent / point_camera.z #focal length fy
                ...
                if len(us) < 8: # 4 corners * 2 u values
                    continue
    ...
    def create_roi_images(self, image, rois):
        ...
        for _, _, min_u, max_u, min_v, max_v in rois:
            ...
            roi_image_resize = cv2.resize(roi_image,(128,128), interpolation=cv2.INTER_LINEAR)
            roi_image_f = roi_image_resize.astype(np.float32)
            roi_images.append(roi_image_f)
